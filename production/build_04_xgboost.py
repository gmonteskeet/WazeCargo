...
cells.append(("md", r"""## 4.7  2026 forecast — top-8 ports by volume"""))
...
for port, direction in top8_keys:
    ...
    _fc = U.forecast_2026(_dp, xgb_fit_only, xgb_predict_one)
    if len(_fc):
...
for ax, (port, direction) in zip(axes.flatten(), top8_keys):
    ...
    if len(sub):
        ...
        ax.plot(d2, sub["pred_shipment_count"], "o-",
                color="#ff7f0e", lw=1.6, markersize=5, label="XGBoost 2026")
    ax.set_title(f"{port} ({direction})", fontsize=10)
    ax.set_ylabel("Ships / mo", fontsize=9)
    ax.tick_params(axis="x", labelsize=8)
    ax.legend(fontsize=7, loc="upper left")
plt.suptitle("2026 forecast — top-8 ports by volume  (model = xgboost)",
             fontsize=13, fontweight="bold", y=1.005)
plt.tight_layout()
plt.savefig(FIG_DIR / "45_xgb_top8_2026.png")
plt.show()
"""))

cells.append(("md", r"""## 4.8  Take-aways
