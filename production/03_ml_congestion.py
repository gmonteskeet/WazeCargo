"""
WAZE CARGO — Step 3a: Port Congestion Forecasting ML (RDS PostgreSQL)
======================================================================
Reads from:   RDS  waze_cargo.clean_maritime_imports / exports
Writes to:    RDS  waze_cargo.port_monthly_agg
                   waze_cargo.port_features_indexed
                   waze_cargo.port_seasonal_index
                   waze_cargo.port_forecast_params
                   waze_cargo.port_congestion_forecast

Same logic as the DuckDB version — just PostgreSQL syntax.
...
WITH base AS (
    SELECT *,
        ...
        (shipment_count
             - LAG(shipment_count, 12) OVER w)::DOUBLE PRECISION
            / NULLIF(LAG(shipment_count, 12) OVER w, 0)                AS yoy_growth,
        (total_value_usd
             - LAG(total_value_usd, 12) OVER w)
...
with_norm AS (
    SELECT *,
        (shipment_count - MIN(shipment_count) OVER pd)
            / NULLIF(MAX(shipment_count) OVER pd
                   - MIN(shipment_count) OVER pd, 0)                   AS sc_norm,
        (total_value_usd - MIN(total_value_usd) OVER pd)
            / NULLIF(MAX(total_value_usd) OVER pd
                   - MIN(total_value_usd) OVER pd, 0)                  AS v_norm,
...
SQL_SEASONAL = f"""
DROP TABLE IF EXISTS {S}.port_seasonal_index CASCADE;
CREATE TABLE {S}.port_seasonal_index AS
SELECT
...
SQL_PARAMS = f"""
DROP TABLE IF EXISTS {S}.port_forecast_params CASCADE;
CREATE TABLE {S}.port_forecast_params AS
WITH last_p AS (
...
FROM {S}.port_features_indexed f
...
SQL_FORECAST = f"""
DROP TABLE IF EXISTS {S}.port_congestion_forecast CASCADE;
CREATE TABLE {S}.port_congestion_forecast AS
...
grid AS (
    ...
    FROM {S}.port_forecast_params p CROSS JOIN offsets o
),
with_s AS (
    SELECT g.*,
    ...
    FROM grid g
    LEFT JOIN {S}.port_seasonal_index s
           ON s.port = g.port AND s.direction = g.direction
...
computed AS (
    SELECT *,
        ...
        POWER(1 + GREATEST(-0.30, LEAST(0.40,
              COALESCE(NULLIF(avg_yoy_growth,     0), 0.025))), step/12.0) AS gs
    FROM with_s
),
final AS (
    SELECT *,
        LEAST(1.0, GREATEST(0.0,
            base_cong * LEAST(1.5,GREATEST(0.5,sfob)) * LEAST(1.1,GREATEST(0.95,gf))
        )) AS ci
    FROM computed
)
SELECT
    ...
    GREATEST(1, ROUND(avg_shipments * sfob * gs))::INTEGER              AS pred_shipment_count,
    CASE WHEN direction='export'
         THEN ROUND((avg_weight_mt * swt * gv)::NUMERIC, 2)::DOUBLE PRECISION
    END                                                                 AS pred_weight_mt,
    ROUND((avg_qty_per_shipment * avg_shipments * sfob * gs)::NUMERIC,2)::DOUBLE PRECISION
...
def main():
    engine = get_engine()
    steps  = [
        ("port_monthly_agg",        SQL_MONTHLY_AGG),
        ("port_features_indexed",   SQL_FEATURES),
        ("port_seasonal_index",     SQL_SEASONAL),
        ("port_forecast_params",    SQL_PARAMS),
        ("port_congestion_forecast",SQL_FORECAST),
    ]
    log.info("═"*60)
    log.info("  WAZE CARGO — Congestion ML  (RDS PostgreSQL)")
    log.info("═"*60)
    ...
    with engine.connect() as c:
        rows = c.execute(text(
            f"SELECT congestion_label, COUNT(*) n "
            f"FROM {S}.port_congestion_forecast "
            f"GROUP BY 1 ORDER BY 2 DESC"
        )).fetchall()
    log.info("")
    log.info("  port_congestion_forecast labels:")
    for r in rows:
        log.info("    %-12s  %d", r[0], r[1])
    engine.dispose()
    log.info("Congestion ML complete.")
